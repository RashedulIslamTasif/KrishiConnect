...
export default function Chat() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      socket?.to(activeConvo._id).emit('new_message', { conversationId: activeConvo._id, message: data.message });
    } catch (e) { console.error(e); }
  };
      ...
      <div style={S.sidebar}>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {conversations.length === 0 && (
            <div style={{ padding: 24, textAlign: 'center', color: 'var(--muted)', fontSize: 13 }}>No conversations yet.<br />Message a farmer from a product page.</div>
          )}
          {conversations.map(c => {
            ...
            return (
              <div key={c._id} style={S.convoItem(active)} onClick={() => setActiveConvo(c)}>
                <div style={S.avatar}>{other?.name?.[0] || '?'}</div>
                <div style={{ flex: 1, overflow: 'hidden' }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--white)' }}>{other?.name}</div>
